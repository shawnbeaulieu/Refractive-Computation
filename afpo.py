'''
Based on AFPO implementation by David Matthews
'''
...
class AFPO:
        ...
        while len(self.genomes) > c.popSize: #max(pf_size, c.popSize):
            ...
            if self.genomes[ind1].dominates(self.genomes[ind2]):  # ind1 dominates
                ...
                if self.genomes[ind1].distance(self.genomes[ind2]):
                    ...
                    if self.genomes[ind1].nandness >= self.genomes[ind2].nandness: # keep ind1
                        for i in range(ind2, len(self.genomes) - 1):
                            self.genomes[i] = self.genomes.pop(i + 1)
                    else: # keep ind2
                        for i in range(ind1, len(self.genomes) - 1):
    ...
    def findBestGenome(self):
        ...
        for g in self.genomes:
            if best is None:
                best = self.genomes[g]
            if self.genomes[g] is not None and np.sum(self.genomes[g].fitnesses) > best_fit:
                best = self.genomes[g]
                best_fit = np.sum(self.genomes[g].fitnesses)
        #for g in self.genomes:
        #    if best is None:
        #        best = self.genomes[g]
...
#        self.saveAvg()
    ...
    def saveBest(self):
        ...
        try:
            os.remove(f'bests{self.randomSeed}.dat')
        except OSError:
            pass

        f = open(f'bests{self.randomSeed}.dat', 'ab')
        pickle.dump(bestGenome, f)
        f.close()   

 
    def SaveLastGen(self):
        f = open(f'lastGeneration{self.randomSeed}.dat', 'ab')
        pickle.dump(self.genomes, f)
        f.close()

    def SaveGen(self, gen):
        filename = f'gens{self.randomSeed}/gen{gen}.dat'
        f = open(filename, 'ab')
        pickle.dump(self.genomes, f)
        f.close()

    def saveAvg(self):
        f = open(f'avgFitness{self.randomSeed}.dat', 'ab')
