...
def force_YFixed_DiffK_VL(Fx, Fy, N, x, y, D, Lx, y_bot, y_up, stiffness, VL_list, VL_counter):
    ...
    for nn in np.arange(N):
        ...
        if d_up < r_now:
            F = -stiffness[nn] * (1 - d_up / r_now) / (r_now)
            Fup -= F
            Fy[nn] += F
            Ep += 0.5 * stiffness[nn] * (1 - d_up / r_now)**2
            cont_up += 1
            cont += 1
            #dbg.set_trace()
        ...
        if d_bot < r_now:
            F = -stiffness[nn] * (1 - d_bot / r_now) / (r_now)
            Fbot += F
            Fy[nn] -= F
            Ep += 0.5 * stiffness[nn] * (1 - d_bot / r_now)**2
            cont += 1
    ...
    for vl_idx in np.arange(VL_counter):
        ...
        if abs(dy) < Dmn:
            ...
            if abs(dx) < Dmn:
                dmn = np.sqrt(dx**2 + dy**2)
                if dmn < Dmn:
                    #F=-2*stiffness[nn]*stiffness[mm]/(stiffness[nn]+stiffness[mm])*(Dmn/dmn-1)
                    if stiffness[nn] != stiffness[mm]:
                        k = stiffness[nn]*stiffness[mm]/(stiffness[nn]+stiffness[mm])
                    else:
                        k = stiffness[nn]
...
def FIRE_YFixed_ConstV_DiffK(Nt, N, x0, y0, D0, m0, Lx, Ly, stiffness):  

    dt_md = 0.01 * D0[0] * np.sqrt(10.0/11.0) #np.sqrt(k_list[2])
    N_delay = 20
    N_pn_max = 2000
    f_inc = 1.1
    f_dec = 0.5
    ...
    VL_counter = 0
    VL_list, VL_counter, x_save, y_save = VL_YFixed_ConstV(N, x, y, D0, Lx, VL_list, VL_counter, x_save, y_save, 1)
    Fx, Fy, Fup_now, Fbot_now, Ep_now, cont_now, p_now, cont_up = force_YFixed_DiffK_VL(Fx, Fy, N, x, y, D0, Lx, 0, Ly, stiffness, VL_list, VL_counter)
...
def MD_VibrSP_ConstV_Yfixed_DiffK(stiffness, 
                                  B, 
                                  Nt, 
                                  N,
                                  x_ini, 
    ...
    if mark_vibrY == 0:
        ...
        x_in1 = (AV1_F1 * np.sin(Freq_Vibr1 * dt * np.arange(Nt)) + 
                 AV1_F2 * np.sin(Freq_Vibr2 * dt * np.arange(Nt)) + 
                 AV1_F3 * np.sin(Freq_Vibr3 * dt * np.arange(Nt)) +
                 AV1_F4 * np.sin(Freq_Vibr4 * dt * np.arange(Nt)) +
                 x_ini[ind_in1])

        x_in2 = (AV2_F1 * np.sin(Freq_Vibr1 * dt * np.arange(Nt)) + 
                 AV2_F2 * np.sin(Freq_Vibr2 * dt * np.arange(Nt)) + 
                 AV2_F3 * np.sin(Freq_Vibr3 * dt * np.arange(Nt)) +
                 AV2_F4 * np.sin(Freq_Vibr4 * dt * np.arange(Nt)) +
                 x_ini[ind_in2])

        x_s = (Amp_Vibr * np.sin(Freq_Vibr1 * dt * np.arange(Nt)) +
               Amp_Vibr * np.sin(Freq_Vibr2 * dt * np.arange(Nt)) +
               Amp_Vibr * np.sin(Freq_Vibr3 * dt * np.arange(Nt)) +
               Amp_Vibr * np.sin(Freq_Vibr4 * dt * np.arange(Nt)) +
               x_ini[ind_s])

    elif mark_vibrY == 1:

        y_in1 = (AV1_F1 * np.sin(Freq_Vibr1 * dt * np.arange(Nt)) +
                 AV1_F2 * np.sin(Freq_Vibr2 * dt * np.arange(Nt)) +
                 AV1_F3 * np.sin(Freq_Vibr3 * dt * np.arange(Nt)) +
                 AV1_F4 * np.sin(Freq_Vibr4 * dt * np.arange(Nt)) +
                 y_ini[ind_in1])

        y_in2 = (AV2_F1 * np.sin(Freq_Vibr1 * dt * np.arange(Nt)) +    
                 AV2_F2 * np.sin(Freq_Vibr2 * dt * np.arange(Nt)) +    
                 AV2_F3 * np.sin(Freq_Vibr3 * dt * np.arange(Nt)) +
                 AV2_F4 * np.sin(Freq_Vibr4 * dt * np.arange(Nt)) +
                 y_ini[ind_in2])

        y_s = (Amp_Vibr * np.sin(Freq_Vibr1 * dt * np.arange(Nt)) +  
               Amp_Vibr * np.sin(Freq_Vibr2 * dt * np.arange(Nt)) + 
               Amp_Vibr * np.sin(Freq_Vibr3 * dt * np.arange(Nt)) + 
               Amp_Vibr * np.sin(Freq_Vibr4 * dt * np.arange(Nt)) + 
               y_ini[ind_s])
    ...
    VL_list, VL_counter, x_save, y_save = VL_YFixed_ConstV(N, 
                                                           x, 
                                                           y, 
                                                           D0, 
                                                           Lx, 
                                                           ...
                                                           y_save, 
                                                           1)
    Fx, Fy, Fup_now, Fbot_now, Ep_now, cont_now, p_now, cont_up = \
        force_YFixed_DiffK_VL(Fx, 
                              Fy, 
                              N, 
                              x, 
                              y, 
    ...
    for nt in np.arange(Nt):
        ...
        x_out1[nt] = x[ind_out1]
        y_out1[nt] = y[ind_out1]
    ...
    np.save("x_history_10_Freq1p2", x_history)
    np.save("y_history_10_Freq1p2", y_history)
    ...
    if mark_vibrY == 0:
        ...
        freq_fft, fft_in1_f1 = FFT_Fup(int(Nt / 2), x_in1[int(Nt / 2):Nt], dt, Freq_Vibr1)
        freq_fft, fft_in2_f1 = FFT_Fup(int(Nt / 2), x_in2[int(Nt / 2):Nt], dt, Freq_Vibr1)
        freq_fft, fft_in1_f2 = FFT_Fup(int(Nt / 2), x_in1[int(Nt / 2):Nt], dt, Freq_Vibr2)
        freq_fft, fft_in2_f2 = FFT_Fup(int(Nt / 2), x_in2[int(Nt / 2):Nt], dt, Freq_Vibr2)
        freq_fft, fft_in1_f3 = FFT_Fup(int(Nt / 2), x_in1[int(Nt / 2):Nt], dt, Freq_Vibr3)
        freq_fft, fft_in2_f3 = FFT_Fup(int(Nt / 2), x_in2[int(Nt / 2):Nt], dt, Freq_Vibr3)
        freq_fft, fft_in1_f4 = FFT_Fup(int(Nt / 2), x_in1[int(Nt / 2):Nt], dt, Freq_Vibr4)
        freq_fft, fft_in2_f4 = FFT_Fup(int(Nt / 2), x_in2[int(Nt / 2):Nt], dt, Freq_Vibr4)

        freq_fft, fft_s_f1 = FFT_Fup(int(Nt / 2), x_s[int(Nt / 2):Nt], dt, Freq_Vibr1)
        freq_fft, fft_s_f2 = FFT_Fup(int(Nt / 2), x_s[int(Nt / 2):Nt], dt, Freq_Vibr2)
        freq_fft, fft_s_f3 = FFT_Fup(int(Nt / 2), x_s[int(Nt / 2):Nt], dt, Freq_Vibr3)
        freq_fft, fft_s_f4 = FFT_Fup(int(Nt / 2), x_s[int(Nt / 2):Nt], dt, Freq_Vibr4)


    elif mark_vibrY == 1:

        freq_fft, fft_in1_f1 = FFT_Fup(int(Nt / 2), y_in1[int(Nt / 2):Nt], dt, Freq_Vibr1)
        freq_fft, fft_in2_f1 = FFT_Fup(int(Nt / 2), y_in2[int(Nt / 2):Nt], dt, Freq_Vibr1)
        freq_fft, fft_in1_f2 = FFT_Fup(int(Nt / 2), y_in1[int(Nt / 2):Nt], dt, Freq_Vibr2)
        freq_fft, fft_in2_f2 = FFT_Fup(int(Nt / 2), y_in2[int(Nt / 2):Nt], dt, Freq_Vibr2)
        freq_fft, fft_in1_f3 = FFT_Fup(int(Nt / 2), y_in1[int(Nt / 2):Nt], dt, Freq_Vibr3)
        freq_fft, fft_in2_f3 = FFT_Fup(int(Nt / 2), y_in2[int(Nt / 2):Nt], dt, Freq_Vibr3)
        freq_fft, fft_in1_f4 = FFT_Fup(int(Nt / 2), y_in1[int(Nt / 2):Nt], dt, Freq_Vibr4)
        freq_fft, fft_in2_f4 = FFT_Fup(int(Nt / 2), y_in2[int(Nt / 2):Nt], dt, Freq_Vibr4)

        freq_fft, fft_s_f1 = FFT_Fup(int(Nt / 2), y_s[int(Nt / 2):Nt], dt, Freq_Vibr1)
        freq_fft, fft_s_f2 = FFT_Fup(int(Nt / 2), y_s[int(Nt / 2):Nt], dt, Freq_Vibr2)
        freq_fft, fft_s_f3 = FFT_Fup(int(Nt / 2), y_s[int(Nt / 2):Nt], dt, Freq_Vibr3)
        freq_fft, fft_s_f4 = FFT_Fup(int(Nt / 2), y_s[int(Nt / 2):Nt], dt, Freq_Vibr4)

    if mark_resonator == 0:
        freq_fft, fft_x_out1_f1 = FFT_Fup(int(Nt / 2), x_out1[int(Nt / 2):Nt], dt, Freq_Vibr1)
        freq_fft, fft_y_out1_f1 = FFT_Fup(int(Nt / 2), y_out1[int(Nt / 2):Nt], dt, Freq_Vibr1)
        freq_fft, fft_x_out1_f2 = FFT_Fup(int(Nt / 2), x_out1[int(Nt / 2):Nt], dt, Freq_Vibr2)
        freq_fft, fft_y_out1_f2 = FFT_Fup(int(Nt / 2), y_out1[int(Nt / 2):Nt], dt, Freq_Vibr2)
        freq_fft, fft_x_out1_f3 = FFT_Fup(int(Nt / 2), x_out1[int(Nt / 2):Nt], dt, Freq_Vibr3)
        freq_fft, fft_y_out1_f3 = FFT_Fup(int(Nt / 2), y_out1[int(Nt / 2):Nt], dt, Freq_Vibr3)
        freq_fft, fft_x_out1_f4 = FFT_Fup(int(Nt / 2), x_out1[int(Nt / 2):Nt], dt, Freq_Vibr4)
        freq_fft, fft_y_out1_f4 = FFT_Fup(int(Nt / 2), y_out1[int(Nt / 2):Nt], dt, Freq_Vibr4)

    elif mark_resonator == 1:

        freq_fft, fft_x_out1_f1 = \
            Output_resonator_1D(Nt, x_out1[0:Nt], x_ini[ind_out1], m0[ind_out1], Freq_Vibr1, dt)
        freq_fft, fft_y_out1_f1 = \
            Output_resonator_1D(Nt, y_out1[0:Nt], y_ini[ind_out1], m0[ind_out1], Freq_Vibr1, dt)
        freq_fft, fft_x_out1_f2 = \
            Output_resonator_1D(Nt, x_out1[0:Nt], x_ini[ind_out1], m0[ind_out1], Freq_Vibr2, dt)
        freq_fft, fft_y_out1_f2 = \
            Output_resonator_1D(Nt, y_out1[0:Nt], y_ini[ind_out1], m0[ind_out1], Freq_Vibr2, dt)

        freq_fft, fft_x_out1_f3 = \
            Output_resonator_1D(Nt, x_out2[0:Nt], x_ini[ind_out1], m0[ind_out2], Freq_Vibr3, dt)
        freq_fft, fft_y_out1_f3 = \
            Output_resonator_1D(Nt, y_out2[0:Nt], y_ini[ind_out1], m0[ind_out2], Freq_Vibr3, dt)
        freq_fft, fft_x_out1_f4 = \
            Output_resonator_1D(Nt, x_out2[0:Nt], x_ini[ind_out1], m0[ind_out2], Freq_Vibr4, dt)
        freq_fft, fft_y_out1_f4 = \
            Output_resonator_1D(Nt, y_out2[0:Nt], y_ini[ind_out1], m0[ind_out2], Freq_Vibr4, dt)
...
def MD_VibrSP_ConstV_Yfixed_DiffK2(stiffness, B, Nt, N, x_ini, y_ini, D0, m0, L,  Freq_Vibr, Amp_Vibr, ind_s, Amp_Vibr1, ind_in1, Amp_Vibr2, ind_in2, ind_out):
    ...
    if mark_vibrY == 0:
        x_in1 = Amp_Vibr1*np.sin(Freq_Vibr*dt*np.arange(Nt))+x_ini[ind_in1]
        x_in2 = Amp_Vibr2*np.sin(Freq_Vibr*dt*np.arange(Nt))+x_ini[ind_in2]
        x_s = Amp_Vibr*np.sin(Freq_Vibr*dt*np.arange(Nt))+x_ini[ind_s]
    elif mark_vibrY == 1:
        y_in1 = Amp_Vibr1*np.sin(Freq_Vibr*dt*np.arange(Nt))+y_ini[ind_in1]
        y_in2 = Amp_Vibr2*np.sin(Freq_Vibr*dt*np.arange(Nt))+y_ini[ind_in2]
        y_s = Amp_Vibr*np.sin(Freq_Vibr*dt*np.arange(Nt))+y_ini[ind_s]
    ...
    return x_in1-x_ini[ind_in1], x_in2-x_ini[ind_in2], x_s-x_ini[ind_s], x_out-x_ini[ind_out]
